    ...
    IStringConverterService stringConverterService) : ICalculatorService
{
    public ArraySegment<int> ComputeLongestIncreasingSubsequence(string inputSequence)
    {
        try
        {
            ...
            var intArr = Array.ConvertAll(stringArr, int.Parse); // never empty
        ...
        {
            logger.LogError(ex, "Input string array contained invalid integer values.");
            return ArraySegment<int>.Empty;
        }
        catch (Exception ex) when (ex is ArgumentNullException or ArgumentOutOfRangeException or ArgumentException)
        {
